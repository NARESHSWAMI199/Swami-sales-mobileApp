...
const ItemDetail = (props: any) => {
  ...
  // Get item ratings
  useEffect(()=>{
    apiClient.get(`${itemsUrl}ratings/${item.slug}`)
    .then(res => {
  ...
  useEffect(() => {
    ...
    apiClient.post(reviewUrl + 'all', { itemId: item?.id, ...data,pageNumber : 0 })
      .then(res => {
  ...
  const fetchReviews = () => {
    ...
    apiClient.post(reviewUrl + 'all', { itemId: item.id, ...data })
      .then(res => {
        let response = res.data;
  ...
  await apiClient.get(reviewUrl + "like/" + reviewId)
      .then(res => {
          let response = res.data;
  ...
  await apiClient.get(reviewUrl + "dislike/" + reviewId)
      .then(res => {
          let response = res.data;
          setItemReviews((previous : any) => previous.filter((review: any) => {
            let itemReview = review.itemReview;
  ...
  useEffect(() => {
    if (item) {
      apiClient.get(`${storeUrl}-detail/${item.wholesaleId}`)
        .then(res => {
  ...
  const handleRatingSubmit = (rating:Number) => {
    apiClient.post(itemsUrl + `update/ratings`,
      {
        itemId : item.id,
        rating : rating
      })
